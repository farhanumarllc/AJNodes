...
class WP_Theme_Custom extends \Boilerplate {
	 ...
	 * @return string
	 */
	public static function animated_button( $object, $classes = '' ) {
		if ( $object['url'] ) {
			$link  = '';
			$link  = "<a href='" . esc_url( $object['url'] ) . "'";
			$link .= " title='" . esc_html( $object['title'] ) . "'";
	...
	public static function advance_form( $form ) {
		$ajn_form_provider = $form['form_provider'] ?? null;
		if ( 'gform' === $form['form_provider'] ) {
			$ajn_gform = $form['gform'] ?? null;
			if ( $ajn_gform ) {
				echo do_shortcode( '[gravityform id="' . $ajn_gform . '" title=false description=false ajax=true]' );
			}
		} else {
			$ajn_hubspot = $form['hubspot'] ?? null;
			if ( $ajn_hubspot ) {
				echo html_entity_decode( $ajn_hubspot );
			}
		}
		return '';
	}
	...
	public static function nav_fallback() {
		...
		if ( is_user_logged_in() ) {
			?>
			<ul>
				<li> <?php echo esc_html__( 'Go to admin area to create navigation menu', 'AJNodes_td' ); ?></li>
			</ul>
			<?php
		}
	}
	...
	public static function get_author_data( $post_id ) {
		...
		if ( function_exists( 'get_field' ) ) {
			$avatar = get_field( 'ajn_var_author_avatar', 'user_' . $author_id );
		}
		if ( ! $avatar ) {
	...
	public static function get_contrasting_text_color( $bg_color ) {
		...
		if ( $bg_color ) {
			...
			$text_color = $luminance > 0.5 ? '#171615' : '#ffffff';
	...
	public static function the_attachment_image( $image_id, $thumb_size, $args = array(), $hide_backup = false ) {
		...
		if ( empty( $attachment_image ) && ! $hide_backup ) {
			...
			$option_fields    = self::get_fields_escaped( 'option' );
			$default_image_id = $option_fields['ajn_var_theme_default_image'] ?? null;
			...
			if ( $default_image_id ) {
				...
				echo html_entity_decode( '<img class="is-default-image" loading="lazy" src="' . CORNERSTONE_DEFAULT_IMAGE . '" />' );
			}
		} else {
		...
		if ( $post_image_id ) {
			...
			if ( ! $hide_backup ) {
				...
				if ( $default_image_id ) {
					...
					$post_image = '<img class="is-default-image" loading="lazy" src="' . CORNERSTONE_DEFAULT_IMAGE . '" />';
				}
			}
		}
	 ...
	 */
	public static function the_social_icons( $social_icons ) {
		if ( $social_icons ) {
			foreach ( $social_icons as $social_icon ) {
				$acf_fc_layout = $social_icon['acf_fc_layout'];
				$svg_code      = self::get_social_svg( $social_icon['acf_fc_layout'] );
				$social_link   = $social_icon['social_link'];
				if ( $social_link ) {
					echo '<a href="' . esc_url( $social_link ) . '" class="' . esc_html( $acf_fc_layout ) . ' flex-center">';
	 ...
	 */
	public static function enqueue_script( $path, $dependencies = array( 'jquery' ), $local_vars = null, $data = array(), $in_footer = false ) {
		$version = filemtime( get_template_directory() . '/' . $path );
		$name    = explode( '.', basename( $path ) )[0] ?? basename( $path );
		$name    = 'wp-theme-' . $name;
		wp_register_script( $name, get_template_directory_uri() . '/' . $path, $dependencies, $version, $in_footer );
		if ( $local_vars ) {
			wp_localize_script(
				$name,
				$local_vars,
...
class_alias( 'AJNodes\Custom\WP_Theme_Custom', 'AJNodes' );
